    ...
    if (carrinho.length === 0) {
        coluna.innerHTML = `
            <div class="carrinho-vazio">
                ...
                <p>Seu carrinho está vazio.<br>Volte ao <a href="/Menu">Menu</a> e adicione itens.</p>
            </div>`;
        return;
    }

    coluna.innerHTML = carrinho.map((item, index) => {
...
function renderizarResumo(carrinho) {
    ...
    if (carrinho.length === 0) {
        linhas.innerHTML = '<span class="resumo-vazio">Nenhum item adicionado.</span>';
...
function alterarQuantidade(index, delta) {
    ...
    document.getElementById(`preco-${index}`).textContent = formatarPreco(item.preco * item.quantidade);
    renderizarResumo(carrinho);
}
...
function abrirEditar(index) {
    ...
    document.getElementById('modal-editar-descricao').textContent = item.descricao || '';
...
function confirmarEdicao() {
    ...
    const selecionado = document.querySelector('input[name="opcao-editar"]:checked');
    if (!selecionado) {
        fecharModalEditar();
        return;
    }
    ...
    const indexDuplicado = carrinho.findIndex((i, idx) => i.chave === novaChave && idx !== indexEditando);
...
function abrirDetalhe(index) {
    ...
    document.getElementById('modal-preco').textContent = formatarPreco(item.preco);
...
function bloquearScroll() {
    ...
    document.documentElement.style.setProperty('--scrollbar-width', largura + 'px');
